...
Function PHONIC(strWord As String, Optional intMaxLength As Integer = 5, Optional boolZeroPad As Boolean = True, Optional boolExtended As Boolean = False) As String
    ...
    strCode = Replace$(DeleteConsecutiveRepeats(strCode), ".", "")
     ...
     If boolZeroPad And Len(strCode) < intMaxLength - 1 Then
        strCode = strCode & String$(intMaxLength - 1 - Len(strCode), "0")
    End If
    
    If Not boolExtended Then strCode = strFirst & strCode
